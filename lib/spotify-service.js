  ...
  new SpotifyStrategy(
    {
      ...
      callbackURL: `${process.env.HOST}:${process.env.PORT}/spotify/callback`,
    },
...
const isUserAuthenticated = async (req, res, next) => {
  if (req.user) {
    ...
    if (now - cookieDate > 60 * 60 * 1000) {
      try {
        const response = await axios({
          method: "get",
          url: `${process.env.HOST}:${process.env.PORT}/spotify/auth/refresh`,
        });
