...
const songListFormatter = (songList) => {
  ...
  songList.forEach(song => {
    ...
    let b = {
      ...
      author: simp_to_trad(artist_str.substring(0, artist_str.length - 2)),
 ...
 * # matched characters - # character difference
 * This adjusts for cases of cover where the original song name is duplicated entirely
 * @param {*} str1 
 * @param {*} str2 
 */
const matchScorer = (str1, str2) => {
  return strMatchCount(str1, str2) - Math.abs(str1.length - str2.length);
}
...
router.get("/search_song", cacheMiddleWare, async (req, res) => {
  try {
    ...
    var match = data0.reduce((prev, current) => {
      if (matchScorer(matchString, current.name + current.artist) > matchScorer(matchString, prev.name + prev.artist)) {
        return current;
      } else {
        return prev;
      }
...
router.get("/get_lyrics_from_id", cacheMiddleWare, async (req, res) => {
  try {
    ...
    // console.log(data);
    if (data.body.lrc && data.body.lrc.lyric) {
